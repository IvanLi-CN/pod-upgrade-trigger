...
function MockReady({ children }: PropsWithChildren) {
	const [ready, setReady] = useState(() => {
		...
		return (
			(window as typeof window & { __MOCK_ENABLED__?: boolean })
...
export const UpdateMenu: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		expect(
			await canvas.findByText("Pod Upgrade Trigger", { exact: true }),
		).toBeInTheDocument();
		expect(
			await canvas.findByText("v0.9.1", { exact: true }),
		).toBeInTheDocument();
		...
		expect(
			await page.findByRole("button", { name: "立即更新" }),
		).toBeInTheDocument();
		...
		expect(
			await page.findByRole("button", { name: "确认更新" }),
		).toBeInTheDocument();
	},
};
