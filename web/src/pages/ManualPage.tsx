...
import { TaskLogActionLabel } from "../components/TaskLogActionLabel";
...
import { type PresencePhase, usePresence } from "../hooks/usePresence";
...
export default function ManualPage() {
	...
	const tasksDrawerEffectiveView = tasksDrawerOpen
		? tasksDrawerView
		: tasksDrawerSnapshot.view;
	const tasksDrawerEffectiveTaskId = tasksDrawerOpen
		? urlTaskId
		: tasksDrawerSnapshot.taskId;
	...
	useEffect(() => {
		...
		(async () => {
			...
			try {
				const data = await getJson<ManualServicesResponse>(
					"/api/manual/services",
				);
				if (cancelled) return;
	...
	const handleDeployAll = async (event: FormEvent) => {
		...
		try {
			...
			const response = await postJson<ManualDeployResponse>(
				"/api/manual/deploy",
				body,
			);
			const deployingCount = Array.isArray(response.deploying)
				? response.deploying.length
				: 0;
	...
	const autoUpdateService =
		services.find((service) => service.is_auto_update === true) ?? null;
	const deployServices = services.filter(
		(service) => service.is_auto_update !== true,
	);
						...
						<div className="flex items-center justify-between gap-4">
							...
							<label className="flex cursor-pointer items-center gap-2 text-xs">
								...
								<input
									...
									onChange={(event) =>
										setAutoUpdateDryRun(event.target.checked)
									}
								/>
							</label>
						</div>
...
function ManualTasksDrawer({
	...
	const [expandedCommandLogs, setExpandedCommandLogs] = useState<
	...
	useEffect(() => {
		...
		const load = async () => {
			...
			try {
				...
				const data = await getJson<TasksListResponse>(
									...
									<section className="space-y-2">
										<div className="flex flex-wrap items-center justify-between gap-2">
											...
											<div className="flex flex-wrap items-center gap-2 text-[11px] text-base-content/60">
												...
												<span>
													起止 ·{" "}
													{formatTs(detail.started_at ?? detail.created_at)} →{" "}
										...
										<div className="flex flex-wrap items-center gap-2 text-[11px]">
											...
											<button
												...
												onClick={() =>
													navigate(
														`/tasks?task_id=${encodeURIComponent(selectedTaskId ?? "")}`,
													)
												}
												disabled={!selectedTaskId}
											>
												<Icon icon="mdi:arrow-right" className="text-base" />在
												Tasks 中打开
											</button>
										</div>
												...
												<div className="space-y-2">
													...
													<div className="space-y-1">
														{timelineLogs.map((log) => {
															...
															const isAutoUpdateRun =
																log.action === "auto-update-run";
															...
															const combinedLines =
																commandMeta &&
																(commandMeta.stdout || commandMeta.stderr)
																	? [
																			...(commandMeta.stdout
																				? commandMeta.stdout
																						.split("\n")
															...
															const dispatchSource =
																dispatchMeta &&
																typeof dispatchMeta.source === "string"
																	? dispatchMeta.source
																	: null;
															const dispatchKind =
																dispatchMeta &&
																typeof dispatchMeta.kind === "string"
																	? dispatchMeta.kind
																	: null;
															const dispatchError =
																dispatchMeta &&
																typeof dispatchMeta.error === "string"
																	? dispatchMeta.error
																	: null;
