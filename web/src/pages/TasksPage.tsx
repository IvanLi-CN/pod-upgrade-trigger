...
function buildMockCommandMetaFallback(
	...
	if (typeof firstUnit !== "string" || !firstUnit.endsWith(".service"))
		return null;
...
export default function TasksPage() {
	...
	const [drawerTaskIdSnapshot, setDrawerTaskIdSnapshot] = useState<
		string | null
	>(urlTaskId);
		...
		const loadDetail = async () => {
			...
			try {
				const data = await getJson<TaskDetailResponse>(
					`/api/tasks/${encodeURIComponent(urlTaskId)}`,
				);
				if (cancelled) return;
				setDrawerTask(data);
				...
				<div className="card-body gap-4">
					<div className="flex flex-wrap items-center justify-between gap-3">
						...
						<div className="flex flex-wrap items-center gap-2 text-[11px] text-base-content/60">
							...
							<span className="badge badge-outline badge-xs">
								<Icon icon="mdi:refresh" className="mr-1 text-sm" />每{" "}
								{Math.round(POLL_INTERVAL_MS / 1000)}s 刷新
							</span>
						</div>
					</div>
								...
								<>
									<section className="space-y-2">
										<div className="flex flex-wrap items-center justify-between gap-2">
											...
											<div className="flex flex-wrap items-center gap-2 text-[11px] text-base-content/60">
												<span>创建 · {formatTs(drawerTask.created_at)}</span>
												<span>
													起止 ·{" "}
													{formatTs(
														drawerTask.started_at ?? drawerTask.created_at,
													)}{" "}
							...
							{drawerTask ? (
								<div className="flex flex-wrap items-center justify-between gap-2">
									<div className="flex flex-wrap items-center gap-2">
										...
										{drawerTask.status === "running" &&
										drawerTask.can_force_stop ? (
											<button
												type="button"
									...
									{mockEnabled ? (
										<span className="text-[10px] text-base-content/60">
											当前在 Mock
											模式下，停止、强制停止和重试仅更新本地模拟数据，不会操作真实系统。
										</span>
									) : null}
								</div>
							) : null}
