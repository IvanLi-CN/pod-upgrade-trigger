  ...
  const rateHint: RateLimitHint = useMemo(() => {
    ...
    if (!recentManuals.length) {
      return {
        ...
        level: 'ok',
        description: 'No recent manual triggers; limits are unlikely to be hit.',
      }
    }
        ...
        <div className="card bg-base-100 shadow">
          <div className="card-body gap-3">
            ...
            <p className="text-xs text-base-content/70">
              /health · /sse/hello · scheduler interval {status.scheduler.intervalSecs}s
            </p>
          </div>
        </div>
        <div className="card bg-base-100 shadow">
          <div className="card-body gap-3">
            ...
            <p className="text-2xl font-semibold">
              tick #{status.scheduler.lastIteration ?? lastScheduler?.id ?? '--'}
            </p>
            <p className="text-xs text-base-content/70">
                ...
                >
                  <div className="flex min-w-0 flex-1 flex-col">
                    <div className="flex items-center gap-2">
                      <span className="badge badge-ghost badge-xs">{event.action}</span>
        ...
        <div className="card bg-base-100 shadow">
          <div className="card-body gap-3">
            ...
            <div className="flex items-start gap-2 text-xs">
              <Icon
                icon={rateHint.level === 'ok' ? 'mdi:check-circle' : 'mdi:alert-circle'}
                className={rateHint.level === 'ok' ? 'text-success' : 'text-warning'}
              />
              <div className="space-y-1">
                <div className="font-semibold text-base-content">{rateHint.title}</div>
                <p className="text-base-content/70">{rateHint.description}</p>
              </div>
            </div>
            <p className="text-[10px] text-base-content/50">
              实际配额来源于 SQLite 中的 rate_limit_tokens 表；当前界面基于事件日志进行近似估算。
            </p>
          </div>
        </div>
      </section>
