...
export default function MaintenancePage() {
	...
	const [resources, setResources] = useState<
		SettingsResources["resources"] | null
	>(null);
	...
	const describeStats = (stats?: FileStats) => {
		...
		const size =
			typeof stats.size === "number" ? `${stats.size} bytes` : "size ?";
		const ts =
			...
			<section id="ratelimit" className="card bg-base-100 shadow">
				<div className="card-body gap-3">
					...
					<p className="text-xs text-base-content/60">
						该操作会删除超过指定时间窗口的 rate_limit_tokens 和 image_locks
						记录，并清理旧版 SQLite/锁文件。
					</p>
				</div>
			</section>
			...
			<section className="card bg-base-100 shadow">
				<div className="card-body gap-3">
					...
					<p className="text-xs text-base-content/70">
						last_payload.bin 仅在最近一次 GitHub HMAC
						签名失败时生成，用于对齐签名算法与原始 payload。
					</p>
					<button
						type="button"
