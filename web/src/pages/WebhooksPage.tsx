...
export default function WebhooksPage() {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				const data = await getJson<WebhooksStatusResponse>(
					"/api/webhooks/status",
				);
				if (!cancelled) {
					await pause();
					...
					<div className="space-y-3">
						{!isReady && (
							<p className="text-xs text-base-content/60">
								Loading config and webhook status…
							</p>
						)}
						{isReady && status?.units?.length ? (
									...
									>
										<div className="flex min-w-0 flex-1 flex-col gap-1">
											...
											<div className="mt-1 flex flex-wrap items-center gap-2 text-[10px] text-base-content/70">
												...
												<span>
													success · {formatTs(unit.last_success_ts ?? null)}
												</span>
												<span>
													failure · {formatTs(unit.last_failure_ts ?? null)}
												</span>
							...
							<p className="text-xs text-base-content/60">
								未检测到任何 GitHub webhook 单元。请检查 PODUP_MANUAL_UNITS 和
								systemd 配置。
							</p>
						)}
					</div>
			...
			<section className="card bg-base-100 shadow-sm">
				<div className="card-body gap-3">
					...
					<div className="alert alert-info mt-2 text-xs">
						...
						<span>
							镜像锁用于保护 GitHub Container Registry
							的请求速率。只有在确认无并发风险时才手动释放。
						</span>
					</div>
				</div>
			</section>
