...
function expectCurrentVersionBetween({
	...
	expect(
		unitEl.compareDocumentPosition(currentEl) &
	...
	if (updateText) {
		...
		expect(
			currentEl.compareDocumentPosition(updateEl) &
				Node.DOCUMENT_POSITION_FOLLOWING,
		).toBeTruthy();
	}
}
...
export const WithGithubPath: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		expect(await canvas.findByText("Demo service")).toBeInTheDocument();
		expect(
			await canvas.findByText("acme/demo/services/demo.service"),
		).toBeInTheDocument();
		expect(await canvas.findByText("已是最新")).toBeInTheDocument();
		expectCurrentVersionBetween({
			canvasElement,
...
export const WithoutDefaultImage: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		expect(
			await canvas.findByPlaceholderText("image (required)"),
		).toBeInTheDocument();
		expectCurrentVersionBetween({
			canvasElement,
			unit: "demo.service",
