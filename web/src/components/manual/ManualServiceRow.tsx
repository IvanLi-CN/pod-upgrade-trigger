...
export function ManualServiceRow({
	service,
	onTrigger,
}: ManualServiceRowProps) {
	...
	const currentTag =
		service.update?.tag?.trim() || extractTagFromImage(service.default_image);
		...
		>
			<div className="flex min-w-0 flex-1 flex-col gap-1">
				<div className="flex items-center gap-2">
					...
					{!defaultImage ? (
						<span className="badge badge-warning badge-xs">缺少镜像</span>
					) : null}
					{currentTag ? (
