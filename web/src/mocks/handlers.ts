...
function shouldAuthFail(pathname: string) {
	...
	return (
		pathname.startsWith("/api") || pathname.startsWith("/github-package-update")
	);
}
...
function authGuard(url: URL) {
	...
	return HttpResponse.json(
		{ error: "unauthorized in mock profile" },
		{ status: 401 },
	);
}
...
function degradedGuard(url: URL) {
	...
	if (url.pathname === "/health")
		return HttpResponse.json({ status: "fail" }, { status: 503 });
	if (url.pathname === "/sse/hello")
		return HttpResponse.json({ error: "sse down" }, { status: 503 });
	return null;
}
...
const handlers = [
	...
	http.get("/api/events", async ({ request }) => {
		...
		if (requestId)
			events = events.filter((e) => e.request_id.includes(requestId));
		if (taskId) {
		...
		if (pathPrefix)
			events = events.filter((e) => (e.path ?? "").startsWith(pathPrefix));
		if (status)
			events = events.filter((e) => String(e.status).startsWith(status));
	...
	http.get("/api/tasks", async ({ request }) => {
		...
		if (unitFilter) {
			...
			tasks = tasks.filter((task) =>
				task.units.some((unit) => {
					const parts = [unit.unit, unit.slug ?? "", unit.display_name ?? ""];
		...
		validateMockResponse(tasksListResponseSchema, response, {
			path: "/api/tasks",
		});
	...
	http.post("/api/tasks", async ({ request }) => {
		...
		const kind =
			(body.kind as TaskDetailResponse["kind"] | undefined) ?? "manual";
		const source =
			(body.source as TaskDetailResponse["trigger"]["source"] | undefined) ??
			"manual";
		const units =
			Array.isArray(body.units) && body.units.length > 0
				? body.units
				: ["unknown.unit"];
	...
	http.post("/api/manual/deploy", async ({ request }) => {
		...
		const deployableServices = services.filter(
			(svc) => svc.is_auto_update !== true,
		);
	...
	http.delete("/api/image-locks/:bucket", async ({ params, request }) => {
		...
		const nextLocks = runtime
			.cloneData()
			.locks.filter((lock) => lock.bucket !== bucket);
		...
		return HttpResponse.json(
			{ bucket, removed, rows: removed ? 1 : 0 },
			{ headers: JSON_HEADERS },
		);
	}),
	...
	http.get("/api/config", async ({ request }) => {
		...
		if (failure) return failure;
		await withLatency();
		return HttpResponse.json(runtime.cloneData().config, {
			headers: JSON_HEADERS,
		});
	}),
