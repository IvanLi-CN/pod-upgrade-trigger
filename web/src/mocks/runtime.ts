...
function buildWebhooks(now: number, profile: MockProfile): WebhooksStatus {
	const baseUnits: WebhookUnit[] = [
		...
		},
		{
			...
			hmac_last_error:
				profile !== "rate-limit-hot" ? null : "signature mismatch",
		},
	];
...
function buildLocks(now: number, profile: MockProfile): LockEntry[] {
	...
	if (profile === "rate-limit-hot") {
		return [
			{
				bucket: "ghcr.io/example/svc-alpha",
				acquired_at: now - 120,
				age_secs: 120,
			},
			{
				bucket: "ghcr.io/example/svc-beta",
				acquired_at: now - 300,
				age_secs: 300,
			},
			{
				bucket: "ghcr.io/example/runner",
				acquired_at: now - 900,
				age_secs: 900,
			},
		];
...
function buildTasks(
	...
	const addTask = (
		partial: Omit<Task, "id" | "task_id" | "unit_counts"> & {
			task_id?: string;
		},
		...
		[
			...
			},
			{
				...
				meta: {
					...
					result_status: "ok",
					result_message:
						"remote=sha256:2222… pulled=sha256:2222… running=sha256:2222…",
				},
			},
		],
	);
		...
		addTask(
			{
				...
				summary:
					"Manual service failure demo · meta.result_message (svc-alpha)",
				trigger: {
					source: "manual",
	...
	addTask(
			...
			units: [
				{
					...
					duration_ms: 19_000,
					message:
						"Auto-update completed with warnings from podman auto-update",
				},
			],
			has_warnings: true,
			warning_count: 2,
		...
		[
			...
			},
			{
				...
				status: "succeeded",
				summary:
					"Auto-update succeeded with 2 warning(s) from podman auto-update",
				unit: "podman-auto-update.service",
				meta: {
					unit: "podman-auto-update.service",
...
class RuntimeStore {
	...
	constructor() {
		...
		const profile =
			profileFromQuery ?? (storedProfile as MockProfile | null) ?? "happy-path";
	...
	updateTask(taskId: string, patch: Partial<Task>) {
		...
		this.#data.tasks = this.#data.tasks.map((task) =>
			task.task_id === taskId
				? { ...task, ...patch, updated_at: updatedAt }
				: task,
		);
		this.#notify();
	}
	...
	createRetryTask(originalTaskId: string): Task | null {
		const original = this.#data.tasks.find(
			(task) => task.task_id === originalTaskId,
		);
		if (!original) return null;

		const now = Math.floor(Date.now() / 1000);
		const maxId = this.#data.tasks.reduce(
	...
	createAdHocTask(input: {
		...
		if (input.kind === "manual" && input.units.length > 0) {
			...
			setTimeout(() => {
				...
				const imagePullEntry = existingLogs.find(
					(log) => log.action === "image-pull",
				);
				if (imagePullEntry) {
					const meta = (
						imagePullEntry.meta && typeof imagePullEntry.meta === "object"
			...
			setTimeout(() => {
				...
				const createdLog = currentLogs.find(
					(log) => log.action === "task-created",
				);
				if (createdLog) {
