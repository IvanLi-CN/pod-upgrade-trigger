...
export function useVersionCheck(): VersionInfo {
	...
	const [info, setInfo] = useState<VersionInfo>({
		current: undefined,
		latest: initialLatestTag
			? { releaseTag: initialLatestTag, publishedAt: null }
			: undefined,
		hasUpdate: null,
	...
	const persistSnapshot = useCallback(
		(lastCheckedAt: number | null, latestTag: string | null) => {
			try {
				if (lastCheckedAt) {
	...
	const fetchVersion = useCallback(async () => {
		...
		try {
			...
			setInfo({
				...
				hasUpdate:
					typeof data.has_update === "boolean" ? data.has_update : null,
