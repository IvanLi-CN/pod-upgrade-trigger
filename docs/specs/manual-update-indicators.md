...
2. **tag 外有更新**：服务配置的 tag 没有变化（或已跟上），但 `:latest` 指向的镜像与当前 tag 指向的镜像不同；此时仅提示“更高通道/最新有变化”，但不要求在本次方案中提供“改 tag 升级”的能力。
...
1. 在 Manual 的服务列表项上展示更新状态标记，并区分：
   - `tag_update_available`（tag 内有更新，可直接触发更新解决）
   - `latest_ahead`（tag 外有更新，仅提示）
   - `up_to_date`（无更新）
   - `unknown`（无法判定）
...
1. 主人打开 `/manual`：
   ...
   - 列表项显示：
     - `tag_update_available`：显著提示“同 tag 有更新”，并暗示可直接触发更新。
     - `latest_ahead`：提示“latest 已不同”（或“最新通道有变化”）。
...
- `tag_update_available`：例如 “Update available (tag)” / “同 tag 有更新”
- `latest_ahead`：例如 “Latest differs” / “latest 有变化”
