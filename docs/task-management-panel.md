...
- 在启动“长时间任务”时，前端应自动弹出抽屉，切换到新建任务的详情视图：
  - 典型场景：
    - 手动触发多 unit 更新（Maintenance / Services 页面发起）；
...
- 打开：
  ...
  - 从其他页面（例如 Services 页面）启动任务时，不强制导航离开当前页面，而是优先使用抽屉。
- 关闭：
  - 点击抽屉右上角关闭按钮；
  - 点击遮罩区域（如设计需要）；
  - 关闭仅影响前端视图，不会停止任务本身。
...
- 用途：由前端在 Services/Maintenance 等页面创建长耗时任务。
...
```jsonc
{
  ...
  "path": "/api/manual/deploy",  // 可选，来源路径
